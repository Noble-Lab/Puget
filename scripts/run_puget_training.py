...
from puget.decoder_data import get_seq_hic_new_dataloader
from puget.decoder_model import TransformerMulti2DNewRegressor
...
def train_model(cfg):
    ...
    seq_weights_path = getattr(cfg, "seq_weights_path", None)

    train_loader = get_seq_hic_new_dataloader(
        hic_x_path=cfg.train_hic,
        seq_x_path=cfg.train_seq,
        y_path=cfg.train_y,
        ...
        seq_weights_path=seq_weights_path
    )
    
    val_loader = get_seq_hic_new_dataloader(
        hic_x_path=cfg.val_hic,
        seq_x_path=cfg.val_seq,
        y_path=cfg.val_y,
        batch_size=cfg.batch_size,
        shuffle=False,
        num_workers=cfg.num_workers,
    ...
    hic_x_hdr = np.load(cfg.train_hic, mmap_mode="r")
    hic_input_dim = hic_x_hdr.shape[-1]
    n_rows = hic_x_hdr.shape[-3]
    n_cols = hic_x_hdr.shape[-2]
    del hic_x_hdr

    seq_input_dim = cfg.embed_dim
    output_dim = 1
    
    print(f"Dimensions - HiC: {hic_input_dim}, Seq: {seq_input_dim}, Out: {output_dim}, Grid: {n_rows}x{n_cols}")
    
    steps_per_train_epoch = math.ceil(len(train_loader.dataset) / cfg.batch_size)
    
    add_layernorm = getattr(cfg, "add_layernorm", False)
    ln_elementwise_affine = getattr(cfg, "ln_elementwise_affine", False)

    model = TransformerMulti2DNewRegressor(
        hic_input_dim=hic_input_dim,
        seq_input_dim=seq_input_dim,
        output_dim=output_dim,
        steps_per_train_epoch=steps_per_train_epoch,
        n_rows=n_rows,
        n_cols=n_cols,
        hic2seq_ratio=cfg.hic2seq_ratio,
        ...
        seq_pad_mode=cfg.seq_pad_mode,
        seq_scale=float(cfg.seq_scale),
...
def main():
    ...
    parser.add_argument(
        "--config", 
        type=str, 
        default=os.path.join(repo_root, "configs/training/puget_train.yaml"),
